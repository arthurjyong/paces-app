...
- **Lab (`/lab`, experimental)** — where new features are trialled before (if ever) reaching the main app; everything there is under research, not polished or final. Experiment 1 is **voice dictation**: record a short take and it is transcribed (Whisper-family models, English forced, medical-glossary biasing) into editable text. Dictation requires the managed sign-in — it spends a server-held transcription key, and the app never exposes server keys anonymously. Voice clips are transcribed by the selected provider and never stored by the app.
