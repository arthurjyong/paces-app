...
Built on Next.js (App Router) + the Anthropic Messages API. The two offered models are routed through **Vercel AI Gateway** (one operator top-up, zero markup); the same adapter can also speak directly to Anthropic / DeepSeek / Moonshot / MiniMax endpoints for BYOK-direct or off-Vercel use. The candidate's browser only ever sees the case stem and the examiner's replies — expected findings, model answers, and the answer key stay server-side.
...
- **Two models via one managed key (Phase 0)** — Claude Sonnet 4.6 (premium, ~$0.30/case) and DeepSeek V4 Pro (budget, ~$0.02–0.05/case — thinking is always-on on the gateway, billed as output), both routed through **Vercel AI Gateway**: one key, one top-up, zero markup. (The multi-provider adapter underneath also supports direct Anthropic/DeepSeek/Kimi/MiniMax endpoints for BYOK-direct or off-Vercel use.)
...
The app owner can whitelist a handful of email addresses; those users sign in via an emailed magic link and the server then uses a server-held key for them. Everyone else still needs their own key — the server keys are unusable without a signed session cookie, and the whole feature is off unless configured. Coverage is per provider: set `DEMO_GATEWAY_API_KEY` (one Vercel AI Gateway key covers both offered models under a single balance); models from uncovered providers politely ask the user to switch or bring their own key. (A DeepSeek-only budget pilot = invite users to pick the DeepSeek model, still funded by the one gateway key.) Full design in `SPEC.md` → "Demo access" + "Multi-provider rules".
...
- `DEMO_GATEWAY_API_KEY` — the recommended server-held key: a Vercel AI Gateway key covering both offered models under one balance (the AI Gateway credit balance is the spend cap — auto-top-up is deliberately OFF, so it's a hard ceiling: when it runs out the invited tier pauses; BYOK is unaffected). Direct-provider keys `DEMO_ANTHROPIC_API_KEY` / `DEMO_DEEPSEEK_API_KEY` / `DEMO_MOONSHOT_API_KEY` / `DEMO_MINIMAX_API_KEY` still work (BYOK-direct / off-Vercel); demo mode is on when at least one of any of these is set. **Use spend-capped keys.**
