...
- **Model picker with a budget tier** — Sonnet 4.6 by default (~$0.30/case), Opus 4.8 / Haiku 4.5 selectable, plus budget models on Anthropic-compatible endpoints: DeepSeek V4 Flash/Pro (~$0.01–0.02/case), Kimi K2.6 (~$0.15), MiniMax M3 (~$0.06). Cost is the user's dial.
...
Then open the app, pick a model in **Settings**, and paste an API key for that model's provider (stored only in your browser, sent per-request, never saved server-side — each provider gets its own saved slot). Keys come from the provider's console: Anthropic (console.anthropic.com), DeepSeek (platform.deepseek.com — top-up via card/PayPal), Moonshot/Kimi (platform.moonshot.ai — min top-up $1; $10 unlocks usable rate limits), MiniMax (platform.minimax.io). Then pick a case and begin.
...
- `DEMO_ANTHROPIC_API_KEY` / `DEMO_DEEPSEEK_API_KEY` / `DEMO_MOONSHOT_API_KEY` / `DEMO_MINIMAX_API_KEY` — the server-held keys, one per provider, each optional; demo mode is on when at least one is set. **Use spend-capped keys** (e.g. Anthropic Console → Workspaces → spend limit; other providers: prepaid balance is the cap) so the worst case is bounded.
